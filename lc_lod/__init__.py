...
def write_record_subjects(record, subjects, f):
    with open('improvedMODS/' + f.replace(':', '_') + '.xml', 'w') as MODS_out:
        ...
        # write new records
        MODS_out.write(
            etree.tostring(record, pretty_print=True, xml_declaration=True, encoding="UTF-8").decode('utf-8'))
...
class uri_lookup:
    ...
    def tgm(self, keyword=None, record_PID=None):
        ...
        tgm_lookup = requests.get(
            'http://id.loc.gov/vocabulary/graphicMaterials/label/{0}'.format(keyword.replace(' ', '%20')),
            timeout=5)
        # request successful
        ...
        # anything else
            ...
            logging.warning(
                'Other status code - {0} ; [{1}]--{2}'.format(tgm_lookup.status_code, record_PID, 'tgm:' + keyword))
            return None
    ...
    def lcsh(self, keyword=None, record_PID=None):
        ...
        lcsh_lookup = requests.get(
            'http://id.loc.gov/authorities/subjects/label/{0}'.format(keyword.replace(' ', '%20')),
            timeout=5)
        # request successful
        ...
        # anything else
            ...
            logging.warning(
                'Other status code - {0} ; [{1}]--{2}'.format(tgm_lookup.status_code, record_PID, 'lcsh:' + keyword))
            return None
